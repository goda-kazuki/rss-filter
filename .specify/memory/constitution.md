...
spec.mdのステータスフィールドは、開発フェーズに応じて以下のように更新する:

| ステータス | タイミング | 説明 |
|-----------|-----------|------|
| **ドラフト** | /speckit.specify 完了時 | 仕様作成完了、レビュー前 |
| **計画中** | /speckit.plan 完了時 | 実装計画作成完了、レビュー前 |
| **実装中** | /speckit.implement 開始時 | コード実装進行中 |
| **レビュー待ち** | PR作成時 | コードレビュー依頼中 |
| **完了** | PRマージ後 | 本番デプロイ可能 |
| **非推奨** | 新spec作成時 | 新しい仕様に置き換え |

**レビュータイミング**:
- 仕様レビューPR: specify完了後、実装前に作成
- 実装レビューPR: 実装完了後に作成（段階的PR推奨）

**根拠**: 
- 仕様段階でのレビューにより、実装の手戻りを防ぐ
- ステータスの明確化により、進捗状況を可視化
- 早期のフィードバックで品質向上
